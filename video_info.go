...
func GetVideoInfoFromID(id string) (*VideoInfo, error) {
	u, _ := url.ParseRequestURI(PreHttpAgent+youtubeBaseURL)
	values := u.Query()
	values.Set("v", id)
	...
	b, err := Base64Decode(string(body))
	if err != nil {
		return nil, err
	}
	body = []byte(b)
...
func getVideoInfoFromHTML(id string, html []byte) (*VideoInfo, error) {
	...
	if len(matches) > 1 {
		...
		resp, err = http.Get(PreHttpAgent+youtubeEmbededBaseURL + id)
		if err != nil {
			return nil, err
		}
		defer resp.Body.Close()
		...
		b, err := Base64Decode(string(html))
		if err != nil {
			return nil, fmt.Errorf("Unable to 64decode return data: %s", err.Error())
		}
		html = []byte(b)
		...
		resp, err = http.Get(PreHttpAgent+youtubeVideoInfoURL + "?" + query.Encode())
		if err != nil {
